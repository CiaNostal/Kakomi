...
- **G-1 は解消済み**（`#previewCanvas` の border → outline 化。`docs/session-log-2026-08-29-4.md`）。
- **次は フェーズ5（E-3）**: 「情報」を他タブと並列のタブに（`#exifFloatCard` を廃し `.tab-button`＋`.tab-pane` へ。
  レール最下部）。E-1 の再クリック収納が自動で効く。
