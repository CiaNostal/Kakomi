...
**バケット3 — プリセット（要モックアップ。E-8 の用語＝キャンバス／写真のトリミング で確定済み）:**
...
他のセグメント系と揃える。F-5 の入れ子もこの見た目に乗せる。ラベルは E-8 の用語（キャンバス／写真のトリミング）で
済んでいるので、バケット3 で二重四角アイコン（`#i-canvas` / `#i-photo-crop`）を各行頭に付ける。バケット3 専用の
モックアップを作る（A-12 のモックアップには含めなかった＝フォーム整形は後回しにした）。
...
E-8 でセクション `label` は「キャンバス／写真のトリミング／背景／フレーム／テキスト」に確定済み。F-3 と同じモックアップで。
...
- **フェーズ7 は改訂版（2026-08-28）** で進行中（上の「フェーズ7 — 改訂版」参照）。**バケット1（G-4 / G-5 / A-10 /
  A-11 / A-13 / G-6）＋ バケット2（E-8 / A-12）は実装・Playwright 検証済み**（`docs/session-log-2026-08-28-2.md`／
  `-2026-08-28-3.md`。バケット1 は目視済み、バケット2 の目視は次回）。**次はバケット3（F-3 保存フォームの縦リスト化 /
  F-4 プリセット名の連番衝突回避 / F-5 保存項目の2階層チェック。要モックアップ。E-8 の用語は確定済みなので
  「キャンバス／写真のトリミング」で作る）** → バケット4（D-1・D-3。独立フェーズ・要モックアップ）
