...
- **フェーズ7 は改訂版（2026-08-28）** で進行中（上の「フェーズ7 — 改訂版」参照）。**バケット1〜3.5（G-4 / G-5 /
  ...
  2026-08-28 セッションはここでクローズ）。**次はバケット4（D-1・D-3 テキスト追加ワークフロー再設計。独立フェーズ・
  スコープB＝データモデルを単一 `textLayers[]`＋`kind` に統合・要モックアップ・localStorage プリセットの移行関数が
  要る）** → バケット5（B-6 登録のみ）→ 積み残し（A-5 / A-4 / A-3 / C-1）。
