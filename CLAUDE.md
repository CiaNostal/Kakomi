...
- **次はフェーズ6＝追加の小〜中改修**（2026-08-27 その2セッションのフィードバック。方向性合意済み・`docs/roadmap.md` フェーズ6／各 ID の詳細ブロック。まだ未実装）: **E-6** Favicon（`.brand-mark` の CSS 図形と同意匠の `favicon.svg`）／**B-5** 背景タイプ（単色／ぼかし）をフレームタブと同じ `.corner-segmented` アイコンセグメントに／**D-5** 文字タブがアクティブで自由テキスト選択中に Delete/Backspace で削除／**G-2** 別画像ロード時は「トリミングだけ初期化」（`cropSettings.rect`→全体・`photoViewParams`→中央、比率制約は維持して再フィット。背景・フレーム・出力比率・余白・テキストは引き継ぐ。初回ロードでは初期化しない）／**E-7** 画像ドロップ領域をキャンバス中央に統合（`.image-loader-container` を中央ダイアログ化、`state.image` で出し分け、`.canvas-area` 全体をドロップ対象に、上部バーに「画像を開く」ボタン追加）／**F-2** プリセット保存時にタブ単位5チェック（出力フォーマット／トリミング／背景／フレーム／テキスト）で保存項目を選択、適用は含まれる項目だけ上書き。安いものから: E-6 → B-5 → D-5 → G-2 → E-7 → F-2。
- その後 **フェーズ7＝編集機能の拡張**（積み残し。C-1 / A-5 / D-1・D-3 / A-4 / A-3）。D-1・D-3 はデータモデル統合まで踏み込むかでスコープが大きく変わる。
