...
- **ロードマップ（`docs/roadmap.md`）のフェーズ0＋1を実施**（`docs/session-log-2026-08-29.md`）。
  - フェーズ0（設計不要の小改修）: E（ダウンロードボタンを上部バー→「出力」タブへ）、A-2（crop モードに三分割グリッド）、A-6（select モードのホイール余白変更を削除）、D-2（テキストの固定表示位置アンカー選択 UI を撤去。`position` データモデルは維持）。
  - フェーズ1（「開いているタブでプレビュー上のドラッグの意味を切り替える」共通設計）: `tabManager.getActiveTab()` を新設し `canvasInteraction.js` の pointerdown で本体ドラッグの振り先を分岐。**C-2**（フレームタブ＋影有効のとき本体ドラッグ→影オフセット、新 `js/interaction/adapters/shadowAdapter.js`）と、背景タブでキャンバス全面が背景の位置調整面になる拡張。**B-2（背景タブでホイール→背景拡大）はユーザー判断で不採用**。プレビュー上のホイールは未使用のまま。
  - フェーズ1追補（移動・配置の操作性）: Shift ドラッグで軸ロック（縦だけ／横だけ）、背景・影のドラッグで 0 付近スナップ＋赤い中央ガイド（`originSnapPx`）、モード切替（select↔crop）を「移動量 < 4px かつ 押下時間 < 400ms（`CLICK_TAP_MS`）」の短いタップ限定に。「背景」タブ以外で余白ドラッグしても背景が動く不具合も修正（背景操作は背景タブ限定）。ダブルクリックでのリセットは見送り。さらに: `tabManager.onTabChange` を追加し、「背景」「フレーム」タブへ移ったら写真の選択（四隅マーカー）を解除／「背景」「フレーム（影オン）」タブでは矢印キーを背景・影のオフセット微調整に割り当て。
  - あわせて `CLAUDE.md` の検証環境の記述を整理（この端末の常設フォルダ方式が主。Docker は WSL インテグレーション無効で不可）。
...
- **次セッションはフェーズ2**（A-1 出力フォーマット UI のグラフィカル化＋位置スライダー撤去、B-1 背景タブのスライダー整理を `artifact-design` でモックアップしてから実装。F プリセットの置き場所も同じ流れ）。ロードマップ全体の進め方は `docs/roadmap.md` 冒頭「進捗」節、トリミングというテーマ内の残タスクは `docs/session-log-2026-08-28.md` 8節、UI刷新の積み残しは `spec.md` 11.5節参照。
