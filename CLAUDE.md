...
- **フェーズ7 は改訂版で進行中（`docs/session-log-2026-08-28-2.md`）**: 追加フィードバック11件をバケット分け。**バケット1＝G-4（カスタム比率でフォーカスが飛ぶ不具合）/ G-5（比率固定トリミング枠が写真の端を越えて拡大できる不具合）/ A-10（「余白」→「大きさ」表記＋スライダー反転。内部 `baseMarginPercent` 不変）/ A-11（トリミング「オリジナル」＝元画像のアスペクト比で固定。Lightroom と同義）/ A-13（「トリミング」パネルのクリックで crop モードへ、Enter でも crop を抜ける）／G-6（A-13 由来の不具合2件: crop モード中のパネル再クリックで画像が無限拡大／比率タイル連打でクロップ枠が 1px へ収束。前者＝`requestEnterCropMode` を crop 中 no-op、後者＝`applyCropAspect` を内接 `fitRectToAspect` → 外接 `growRectToAspect` に）まで実装・Playwright 検証済み（phase7b1 59/59、周辺回帰 16/16）。目視は次回。** 残り＝バケット2（E-8 用語統一 / A-12 レイアウトタブ3セクション＋二重四角アイコン。要モックアップ）→ バケット3（F-3/F-4/F-5 プリセット。要モックアップ）→ バケット4（D-1・D-3 テキスト。独立フェーズ・スコープB＝データモデル統合・要モックアップ）→ バケット5（B-6 背景に別画像、登録のみ）→ 積み残し（A-5 / A-4 / A-3 / C-1）。詳細は `docs/roadmap.md`「フェーズ7 — 改訂版」。
